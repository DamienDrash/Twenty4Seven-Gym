...
CREATE UNIQUE INDEX IF NOT EXISTS idx_access_codes_window_active
    ON access_codes (access_window_id) WHERE status IN ('pending','provisioned','emailed');
...
CREATE TABLE IF NOT EXISTS access_window_checkouts (
    ...
    access_window_id BIGINT NOT NULL UNIQUE REFERENCES access_windows(id) ON DELETE CASCADE,
    member_id BIGINT NOT NULL REFERENCES members(id) ON DELETE CASCADE,
    source TEXT NOT NULL DEFAULT 'checks-funnel',
    checklist JSONB NOT NULL DEFAULT '[]'::jsonb,
    confirmed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
_AW_SELECT = """
    ...
    LEFT JOIN access_window_checkins awc ON awc.access_window_id = aw.id
"""
...
class Database:
    ...
    def get_user_by_id(self, user_id: int) -> dict[str, Any] | None:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT id, email, password_hash, role, is_active FROM users WHERE id = %s",
                (user_id,),
            )
            return cur.fetchone()
    ...
    def list_users(self, *, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT id, email, role, is_active FROM users ORDER BY email LIMIT %s OFFSET %s",
                (limit, offset),
            )
            return list(cur.fetchall())

    def create_user(
        self, *, email: str, password: str, role: UserRole, is_active: bool,
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
    ...
    def update_user(self, *, user_id: int, role: UserRole, is_active: bool) -> dict[str, Any] | None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    UPDATE users SET role=%s, is_active=%s, updated_at=NOW()
                    WHERE id=%s RETURNING id, email, role, is_active
                    """,
    ...
    def upsert_member(self, customer: dict[str, Any]) -> int:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO members (magicline_customer_id, email, first_name, last_name,
                                         status, raw_payload, last_synced_at)
                    VALUES (%s, %s, %s, %s, %s, %s, NOW())
                    ON CONFLICT (magicline_customer_id) DO UPDATE SET
                    ...
                    RETURNING id
                    """,
                    (customer["id"], customer.get("email"), customer.get("first_name"),
                     customer.get("last_name"), customer.get("status"), Json(customer)),
                )
    ...
    def list_members(
        self, *, email_filter: str | None = None, limit: int = 100, offset: int = 0,
    ) -> list[dict[str, Any]]:
        base = """
        ...
        params: list[Any] = []
        if email_filter:
            base += " WHERE lower(m.email) LIKE lower(%s)"
            params.append(f"%{email_filter}%")
        base += " ORDER BY m.last_synced_at DESC, m.id DESC LIMIT %s OFFSET %s"
    ...
    def upsert_entitlement(
        self, *, member_id: int, has_xxlarge: bool,
        has_free_training_product: bool, raw_source: dict[str, Any],
    ) -> None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO member_entitlements
                        (member_id, has_xxlarge, has_free_training_product, raw_source, last_synced_at)
                    VALUES (%s, %s, %s, %s, NOW())
                    ON CONFLICT (member_id) DO UPDATE SET
                        has_xxlarge=EXCLUDED.has_xxlarge,
                        has_free_training_product=EXCLUDED.has_free_training_product,
                        raw_source=EXCLUDED.raw_source, last_synced_at=NOW()
                    """,
    ...
    def upsert_booking(
        self, *, member_id: int, booking: dict[str, Any], source_received_at: datetime,
    ) -> int:
        ml_updated = booking.get("updatedAt") or booking.get("lastModifiedAt")
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    ...
                    ON CONFLICT (magicline_booking_id) DO UPDATE SET
                    ...
                    RETURNING id
                    """,
                    (booking["booking_id"], member_id, booking["title"],
                     booking["booking_status"], booking.get("appointment_status"),
                     booking.get("participant_status"), booking["start_date_time"],
                     booking["end_date_time"], ml_updated, source_received_at, Json(booking)),
                )
    ...
    def list_member_bookings(self, *, member_id: int) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                FROM bookings WHERE member_id=%s
    ...
    # ── Access windows (DRY query composition) ────────────────────
    ...
    def list_member_access_windows(self, *, member_id: int) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                ORDER BY aw.starts_at DESC LIMIT 100
                """,
                (member_id,),
            )
            return list(cur.fetchall())
    ...
    def upsert_access_window(
        self, *, member_id: int, booking_id: int, booking_ids: list[int],
        booking_count: int, starts_at: datetime, ends_at: datetime,
        dispatch_at: datetime, status: str, access_reason: str,
    ) -> int:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO access_windows (
                        ...
                        starts_at, ends_at, dispatch_at, status, access_reason, last_computed_at
    ...
    def expire_finished_windows(self, now: datetime) -> int:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE access_windows SET status=%s WHERE status IN (%s,%s) AND ends_at < %s",
                    (AccessWindowStatus.EXPIRED, AccessWindowStatus.SCHEDULED, AccessWindowStatus.ACTIVE, now),
                )
                count = cur.rowcount
                cur.execute(
                    "UPDATE access_codes SET status=%s WHERE status IN (%s,%s) AND expires_at < %s",
                    (AccessCodeStatus.EXPIRED, AccessCodeStatus.PROVISIONED, AccessCodeStatus.EMAILED, now),
                )
            conn.commit()
            return count
    ...
    def cancel_access_window(self, *, access_window_id: int) -> int:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE access_windows SET status=%s WHERE id=%s",
    ...
    def store_access_code(
        ...
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO access_codes (access_window_id, nuki_auth_id, code_hash,
                        code_last4, status, expires_at, is_emergency)
                    VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id
                    """,
                    (access_window_id, nuki_auth_id, code_hash,
                     raw_code[-4:], status, expires_at, is_emergency),
                )
    ...
    def get_active_code_for_window(self, *, access_window_id: int) -> dict[str, Any] | None:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                FROM access_codes
                WHERE access_window_id=%s AND status IN (%s,%s)
                ORDER BY created_at DESC LIMIT 1
                """,
                (access_window_id, AccessCodeStatus.PROVISIONED, AccessCodeStatus.EMAILED),
            )
            return cur.fetchone()
    ...
    def mark_code_emailed(self, code_id: int) -> None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE access_codes SET status=%s, emailed_at=NOW(), activated_at=NOW() WHERE id=%s",
                    (AccessCodeStatus.EMAILED, code_id),
                )
            conn.commit()

    def mark_code_replaced(self, *, code_id: int, replaced_by_code_id: int | None = None) -> None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE access_codes SET status=%s, replaced_by_code_id=%s WHERE id=%s",
    ...
    def sync_window_code_expiry(self, *, access_window_id: int, expires_at: datetime) -> int:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE access_codes SET expires_at=%s WHERE access_window_id=%s AND status IN (%s,%s)",
                    (expires_at, access_window_id, AccessCodeStatus.PROVISIONED, AccessCodeStatus.EMAILED),
                )
                updated = cur.rowcount
            conn.commit()
            return updated
    ...
    def list_member_access_codes(self, *, member_id: int) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                WHERE aw.member_id=%s ORDER BY ac.created_at DESC LIMIT 100
                """,
                (member_id,),
            )
            return list(cur.fetchall())
    ...
    def verify_member_access_code(
        self, *, email: str, raw_code: str, now: datetime,
    ) -> dict[str, Any] | None:
        ...
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                ORDER BY aw.starts_at ASC, ac.created_at DESC
                """,
                (email, now,
                 AccessWindowStatus.SCHEDULED, AccessWindowStatus.ACTIVE,
                 AccessCodeStatus.PROVISIONED, AccessCodeStatus.EMAILED),
            )
    ...
    def is_code_recently_used(self, raw_code: str, days: int = 180) -> bool:
        ...
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT code_hash FROM access_codes WHERE code_last4=%s AND created_at > NOW() - INTERVAL '%s days'",
                (raw_code[-4:], days),
            )
            return any(verify_password(raw_code, r["code_hash"]) for r in cur.fetchall())
        ...
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                SELECT aw.id AS access_window_id, aw.member_id, aw.starts_at, aw.ends_at,
                       ...
                       (awc.confirmed_at IS NOT NULL) AS is_confirmed
                FROM access_windows aw
                JOIN members m ON m.id = aw.member_id
                LEFT JOIN access_window_checkins awc ON awc.access_window_id = aw.id
                WHERE aw.id = %s
    ...
    def upsert_access_window_checkin(
        self, *, access_window_id: int, member_id: int,
        source: str, rules_accepted: bool, checklist: list[dict[str, Any]],
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO access_window_checkins
                        (access_window_id, member_id, source, rules_accepted, checklist, confirmed_at, updated_at)
                    VALUES (%s,%s,%s,%s,%s,NOW(),NOW())
                    ON CONFLICT (access_window_id) DO UPDATE SET
                        source=EXCLUDED.source, rules_accepted=EXCLUDED.rules_accepted,
                        checklist=EXCLUDED.checklist, confirmed_at=NOW(), updated_at=NOW()
    ...
    def upsert_window_checkout(
        self, *, access_window_id: int, member_id: int,
        source: str, checklist: list[dict[str, Any]],
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO access_window_checkouts
                        (access_window_id, member_id, source, checklist, confirmed_at, updated_at)
                    VALUES (%s,%s,%s,%s,NOW(),NOW())
                    ON CONFLICT (access_window_id) DO UPDATE SET
                        source=EXCLUDED.source, checklist=EXCLUDED.checklist,
    ...
    def list_member_windows_with_status(
        self, *, member_id: int, from_dt: datetime,
    ) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                """
                SELECT aw.id, aw.starts_at, aw.ends_at, aw.status,
                       aw.booking_count, aw.access_reason,
                       awci.confirmed_at AS checkin_confirmed_at,
                       awco.confirmed_at AS checkout_confirmed_at
                FROM access_windows aw
                LEFT JOIN access_window_checkins awci ON awci.access_window_id = aw.id
                LEFT JOIN access_window_checkouts awco ON awco.access_window_id = aw.id
                WHERE aw.member_id=%s AND aw.ends_at >= %s AND aw.status IN ('scheduled','active')
                ORDER BY aw.starts_at ASC LIMIT 20
                """,
                (member_id, from_dt),
            )
            return list(cur.fetchall())
    ...
    def create_alert(self, *, severity: str, kind: str, message: str, payload: dict[str, Any] | None = None) -> None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO alerts (severity, kind, message, payload) VALUES (%s,%s,%s,%s)",
                    (severity, kind, message, Json(payload or {})),
                )
            conn.commit()

    def list_alerts(self, *, severity: str | None = None, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        base = "SELECT id, severity, kind, message, created_at FROM alerts"
        params: list[Any] = []
        if severity:
            base += " WHERE severity = %s"
    ...
    def create_admin_action(
        self, *, actor_email: str, action: str,
        access_window_id: int | None = None, access_code_id: int | None = None,
        ...
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO admin_actions (actor_email, action, access_window_id, access_code_id, payload)
                    VALUES (%s,%s,%s,%s,%s)
                    """,
    ...
    def list_admin_actions(self, *, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                """
                SELECT id, actor_email, action, access_window_id, access_code_id, payload, created_at
                FROM admin_actions ORDER BY created_at DESC LIMIT %s OFFSET %s
                """,
                (limit, offset),
            )
            return list(cur.fetchall())
    ...
    def list_lock_events(self, *, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                SELECT id, actor_email, action, access_window_id, access_code_id, payload, created_at
                FROM admin_actions
                WHERE action IN ('remote-open','resend-access-code','issue-emergency-code','deactivate-access-window')
    ...
    def record_webhook_event(
        self, *, provider: str, event_id: str, event_type: str | None, payload: dict[str, Any],
    ) -> bool:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    VALUES (%s,%s,%s,%s) ON CONFLICT (provider, event_id) DO NOTHING RETURNING id
                    """,
    ...
    def consume_password_reset_token(self, *, token_hash: str, password: str, now: datetime) -> dict[str, Any] | None:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT user_id FROM password_reset_tokens WHERE token_hash=%s AND used_at IS NULL AND expires_at >= %s LIMIT 1",
                    (token_hash, now),
                )
                row = cur.fetchone()
                if not row:
                ...
                cur.execute("UPDATE password_reset_tokens SET used_at=NOW() WHERE token_hash=%s", (token_hash,))
                cur.execute(
                    "UPDATE users SET password_hash=%s, updated_at=NOW() WHERE id=%s RETURNING id, email, role, is_active",
                    (hash_password(password), user_id),
                )
                user = cur.fetchone()
            conn.commit()
    ...
    # ── Funnel templates & steps ──────────────────────────────────

    def list_funnel_templates(self) -> list[dict[str, Any]]:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute("SELECT id, name, slug, funnel_type FROM funnel_templates ORDER BY funnel_type, name")
            return list(cur.fetchall())

    def get_funnel_template_detail(self, template_id: int) -> dict[str, Any] | None:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute("SELECT id, name, slug, funnel_type, description FROM funnel_templates WHERE id=%s", (template_id,))
            tpl = cur.fetchone()
            if not tpl:
                return None
            cur.execute(
    ...
    def upsert_funnel_template(
        self, *, template_id: int | None, name: str, slug: str, funnel_type: str, description: str | None,
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                if template_id:
                    cur.execute(
                        "UPDATE funnel_templates SET name=%s, slug=%s, funnel_type=%s, description=%s, updated_at=NOW() WHERE id=%s RETURNING id, name, slug, funnel_type, description",
                        (name, slug, funnel_type, description, template_id),
                    )
                else:
                    cur.execute(
                        "INSERT INTO funnel_templates (name, slug, funnel_type, description) VALUES (%s,%s,%s,%s) RETURNING id, name, slug, funnel_type, description",
    ...
    def get_funnel_by_type(self, funnel_type: str) -> dict[str, Any] | None:
        with self.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, slug, funnel_type, description FROM funnel_templates WHERE funnel_type=%s ORDER BY created_at DESC LIMIT 1",
                (funnel_type,),
            )
            tpl = cur.fetchone()
            if not tpl:
    ...
    def create_funnel_submission(
        self, *, access_window_id: int, template_id: int, entry_source: str, success: bool,
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO funnel_submissions (access_window_id, template_id, entry_source, success) VALUES (%s,%s,%s,%s) RETURNING id, access_window_id, template_id, entry_source, success, created_at",
    ...
    def create_funnel_step_event(
        self, *, submission_id: int, step_id: int, status: str, note: str | None, photo_path: str | None,
    ) -> dict[str, Any]:
        with self.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO funnel_step_events (submission_id, step_id, status, note, photo_path) VALUES (%s,%s,%s,%s,%s) RETURNING id, submission_id, step_id, status, note, photo_path, created_at",
